...
/*
    returns the square of euclidean distance between the two points
*/
...
void thresholding(BMP &img,const int THRES){
    assert(THRES > 0 && THRES < 255 , THRES);

    
    DFOR(i,theta,img.TellWidth(),img.TellHeight()){
        if(img(i,theta)->Green > THRES)
...
void drawLinesAndCircles(BMP &img,const std::list<circle> &circleVector,const std::list<line> &linesVector){
    ...
    for(auto &i:circleVector){
        DrawCircle(img,i.center_x,i.center_y,i.rad,255);
    }
